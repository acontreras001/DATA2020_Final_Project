...
stargazer::stargazer(general_linear_model, type = "latex",
          ...
          header = FALSE, single.row = TRUE,  # Keep single.row to TRUE for one line per model
          column.labels = c("Estimate", "Std. Error"),  # Define column labels
          star.char.levels = c(0.01, 0.05, 0.1),  # Customize significance levels if needed
          digits = 4,
          style = "default",  # Ensure default styling
          model.numbers = FALSE)  # Remove model numbers if not needed
```

```{r}
# Generate LaTeX code with XTable

model_summary <- summary(general_linear_model)$coefficients

# Sorting the summary by p-values (assuming p-values are in the 4th column)
sorted_model_summary <- model_summary[order(model_summary[, 4]),]

# Convert sorted summary to xtable
latex_table <- xtable::xtable(sorted_model_summary)

# Print LaTeX code with four decimal places, without row names and floating option
