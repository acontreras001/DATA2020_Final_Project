...
vars_for_future_analyses <- c("m3i0m", "f3i0l1", "m3i0l1", "f3i0m", "f3i0h")
...
data <- data %>%
  mutate(f3i2_recode = case_when(
    f3i2 %in% c(-9, -8, -7, -6, -5, -4, -3, -2, -1) ~ as.numeric(factor(f3i2, levels = c(-9, -8, -7, -6, -5, -4, -3, -2, -1))),
    f3i2 %in% 1:10 ~ f3i2,
    f3i2 == 101 ~ 11,
    f3i2 == 102 ~ 12,
    f3i2 == 103 ~ 13,
  ...
  mutate(f3i2_recode = factor(f3i2_recode)) %>% 
  mutate(f3i2_recode = fct_recode(
    f3i2_recode,
    "Not in wave" = "-9",
    "Out of range" = "-8",
    "N/A" = "-7",
    ...
    "Jail" = "9",
    "Homeless" = "10",
    "Military base/dorm" = "11",
    "Public housing" = "12",
    "Lives family/friends" = "13"
  ))


data <- data %>%
  mutate(f3c41_recode = case_when(
    f3c41 %in% c(-9, -8, -7, -6, -5, -4, -3, -2, -1) ~ as.numeric(factor(f3c41, levels = c(-9, -8, -7, -6, -5, -4, -3, -2, -1))),
    f3c41 %in% 1:8 ~ f3i2,
    f3c41 == 101 ~ 9,
    f3c41 == 102 ~ 10,
    f3c41 == 103 ~ 11,
  ...
  mutate(f3c41_recode = factor(f3c41_recode)) %>% 
  mutate(f3c41_recode = fct_recode(
    f3c41_recode,
      "Not in wave" = "-9",
      "Out of range" = "-8",
      "N/A" = "-7",
...
data <- data %>%
  mutate(m3c41_recode = case_when(
    m3c41 %in% c(-9, -8, -7, -6, -5, -4, -3, -2, -1) ~ as.numeric(factor(m3c41, levels = c(-9, -8, -7, -6, -5, -4, -3, -2, -1))),
    m3c41 %in% 1:8 ~ f3i2,
    m3c41 == 101 ~ 9,
    m3c41 == 102 ~ 10,
    m3c41 == 103 ~ 11,
  ...
  mutate(m3c41_recode = factor(m3c41_recode)) %>% 
  mutate(m3c41_recode = fct_recode(
    m3c41_recode,
      "Not in wave" = "-9",
      "Out of range" = "-8",
      "N/A" = "-7",
...
data <- data %>% 
  select(-all_of(c("f3i2", "f3c41", "m3c41")))

data$f3i2 <- data$f3i2_recode
data$f3c41 <- data$f3c41_recode
data$m3c41 <- data$m3c41_recode

data <- data %>% 
  select(-all_of(c("f3i2_recode", "f3c41_recode", "m3c41_recode")))
...
data <- data %>%
  rename(
    ...
    father_vote_nov2000 = f3i0k,
    mother_vote_nov2000 = m3i0k,
...
binary_vars <- c("father_married_to_mother", "father_living_with_mother", "mother_part_labor_union", "mother_part_community_org", 
                 ...
                 "mother_registered_to_vote", "father_par_labor_union", "father_par_community_org", "father_par_children_group", 
                 "father_political_demonstration")
multi_categorical_vars <- c("current_housing_situation", "mother_current_employment_status", "dather_current_employment_status")
