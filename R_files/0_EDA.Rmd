...
# Keep to reference for Poster Presenation
vars_for_future_analyses <- c("m3i0m", "f3i0l1", "m3i0l1", "f3i0m", "m3i0i", "f3i0h")


```

# Remove Negative Ages and Incomes
...
binary_yes_no_1_2 <- c("m3i0d", "m3i0e", "m3i0f", "m3i0g", "f3i0d", 
                       "f3i0e", "f3i0f", "f3i0g", "f3i0k", "m3i0k")
...
# Cleaning the Multi-categorical Data

```{r}

data <- data %>%
  mutate(f3i2 = factor(f3i2)) %>% 
  mutate(f3i2 = fct_recode(
    f3i2,
    "Not in wave" = "-9",
    "Out of range" = "-8",
    "N/A" = "-7",
    ...
    "Jail" = "9",
    "Homeless" = "10",
    "Military base/dorm" = "101",
    "Public housing" = "102",
    "Lives family/friends" = "103"
  ))


data <- data %>%
  mutate(f3c41 = factor(f3c41)) %>% 
  mutate(f3c41 = fct_recode(
    f3c41,
      "Not in wave" = "-9",
      "Out of range" = "-8",
      "N/A" = "-7",
...
data <- data %>%
  mutate(m3c41 = factor(m3c41)) %>% 
  mutate(m3c41 = fct_recode(
    m3c41,
      "Not in wave" = "-9",
      "Out of range" = "-8",
      "N/A" = "-7",
...
# Rename vars and recategorize into vectors

```{r}
data <- data %>%
  rename(
    participant_id = idnum,
    father_age = cf3age,
    mother_age = cm3age,
    household_income_pre_tax = f3l1,
    father_married_to_mother = cf3marm,
    father_living_with_mother = cf3cohm,
    mother_part_labor_union = m3i0d,
    mother_part_community_org = m3i0e,
    mother_part_children_group = m3i0f,
    mother_political_demonstration = m3i0g,
    father_vote_nov2020 = f3i0k,
    mother_vote_nov2020 = m3i0k,
    #mother_registered_to_vote = m3i0i,
    father_par_labor_union = f3i0d,
    father_par_community_org = f3i0e,
    father_par_children_group = f3i0f,
    father_political_demonstration = f3i0g,
    current_housing_situation = f3i2,
    mother_current_employment_status = m3c41,
    father_current_employment_status = f3c41,
...
data <- data %>% 
  select(-all_of(multi_categorical_vars))
...
variable_labels <- setNames(
  c("Father's Age", "Mother's Age", "Household Income (Pre-Tax)"),
  c("father_age", "mother_age", "household_income_pre_tax")
)

# You can view the labels by printing the variable_labels
print(variable_labels)

output_directory <- file.path(getwd(), "outputs/EDA/numeric_vars")

# Loop through each numeric variable to generate and save histograms
for(var in numeric_vars) {
  ...
  p <- data %>%
    ...
    ggtitle(paste("Histogram of", variable_labels[[var]]))  # Use correct variable_labels for title
  ggsave(paste0(output_directory, "/histogram_", var, ".png"), plot = p, width = 10, height = 8, dpi = 300)
}

# Loop through each numeric variable to generate and save boxplots
for(var in numeric_vars) {
  # Generating boxplot for each variable
  p <- data %>%
    select(all_of(var)) %>%
    ggplot(aes(x = 1, y = .data[[var]])) +  # Use a constant x to ensure a single boxplot per plot
    geom_boxplot() +
    ggtitle(paste("Boxplot of", variable_labels[[var]]))  # Use correct variable_labels for title
...
```{r}
# Assuming binary_vars and its corresponding labels vector are defined
binary_vars <- c("father_married_to_mother", "father_living_with_mother", 
                 "mother_part_labor_union", "mother_part_community_org", 
                 "mother_part_children_group", "mother_political_demonstration", 
                 "father_vote_nov2020", "mother_vote_nov2020",
                 "father_par_labor_union", "father_par_community_org", 
                 "father_par_children_group", "father_political_demonstration")

# Define a vector of labels for these variables
labels <- c("Father Married to Mother", "Father Living with Mother",
            "Mother Participates in Labor Union", "Mother Participates in Community Organization",
            "Mother Participates in Children's Group", "Mother Participates in Political Demonstrations",
            "Father Voted in November 2020 Election", "Mother Voted in November 2020 Election",
            "Father Participates in Labor Union", "Father Participates in Community Organization",
            "Father Participates in Children's Group", "Father Participates in Political Demonstrations")

# Use setNames to apply labels to the binary_vars vector
binary_var_labels <- setNames(binary_vars, labels)
...
# Create a labeller function to use labels
#name_labeller <- as_labeller(binary_labels)

# # Bar plots for all categorical variables in one figure with facets
# p <- data %>%
#   select(all_of(binary_vars)) %>%
#   pivot_longer(cols = everything(), names_to = "variable", values_to = "value") %>%
#   ggplot(aes(x = value)) +
#   geom_bar() +
#   facet_wrap(~variable, scales = "free", labeller = name_labeller)  # Applying custom labels
# 
...
for(var in names(binary_vars)) {
  p <- data %>%
    ...
    ggplot(aes(x = value)) +
    geom_bar() +
    labs(title = paste("Bar Plot of", binary_var_labels[var]), x = binary_var_labels[var], y = "Count") +
    theme_minimal()
  
  # Saving each plot with a unique filename based on the variable label
  ggsave(paste0(output_directory, "/barplot_", gsub(" ", "_", binary_var_labels[var]), ".png"), plot = p, width = 10, height = 8, dpi = 300)
}
```
...
output_directory <- file.path(getwd(), "data/processed")

save(data, file = paste0(output_directory, "/EDA_output.RData"))

write.csv(data, file = paste0(output_directory, "/EDA_output.csv"), row.names = FALSE)
...
### Additional code, not necessary but potentially useful in future
...
- `m3i0k`: "Did you vote in the November 2020 election?"
...
- `f3i0k`: "Did you vote in the November 2020 election?"
...
var_labels <- c(
  ...
  "m3i0k" = "Mother: Voted in November 2020 election",
  ...
  "f3i0k" = "Father: Voted in November 2020 election",
